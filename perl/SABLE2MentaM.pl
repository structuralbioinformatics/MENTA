...
#
 ...
 if (defined $help){
  printf " 
           i  INPUT SABLE file
           f  Sequence name specific to obtain the output
           m  Mizuguchi matrix
           b  Buriedness matrix
 ...
 $bur={ 9 => 'A', 8 => 'B', 7 => 'C', 6 => 'D', 5 => 'E', 
  ...
  open (INP,"$input");
    while (<INP>){
     @data=split;
     if ($data[0] eq "Query:"){$query=$data[1];undef @aa;;undef  @ss;undef @bb;}
  ...
  for $i (0..$#ss){
    ...
    if ($strand == 0 && $beta > 0){
        ...
        if ($size < 1) {
         $beta=0;
         push @mizu,"C";
         if ($ss[$i] ne "G" && $ss[$i] ne "H" && $ss[$i] ne "E" && $ss[$i] ne "" && $ss[$i] ne " ") {push @mizu,"C";}
        }else{
    ...
    if ($beta > 0){
	 ...
	 if ($#mizu<$#ss){ push @mizu,"C";}
         ...
         if ($medium== 1){if ($acc_beta <=2){$type="B";}else{$type="E";}}
         if ($large == 1){if ($acc_beta <=2){$type="M";}else{$type="N";}}
	 if ($#mizu<$#ss){ for $j ($b0..($i)){push @mizu,$type;}}
         if ($#mizu<$#ss){ 
         if ($ss[$i] ne "G" && $ss[$i] ne "H" && $ss[$i] ne "E" && $ss[$i] ne "" && $ss[$i] ne " ") {push @mizu,"C";}else{push @mizu,"$type";}
         }
        }
       }
    if ($alfa > 0){
	 ...
	 if ($#mizu<$#ss){ push @mizu,"C";}
         ...
         if ($short == 1){if ($acc_alfa < 3){$type="A";}else{$type="F";}}
         if ($large == 1){if ($acc_alfa/2 < 3){$type="H";}else{$type="I";}}
	 if ($#mizu<$#ss){ for $j ($a0..($i)){push @mizu,$type;}}
	 if ($#mizu<$#ss){
         if ($ss[$i] ne "G" && $ss[$i] ne "H" && $ss[$i] ne "E" && $ss[$i] ne "" && $ss[$i] ne " ") {push @mizu,"C";}else{push @mizu,"$type";}
         }
        }
       }
     if ($alfa==0 && $beta == 0 && $end == 0 && $#mizu<$#ss) {push @mizu,"C";}
   return (@mizu);
} 
