...
float  CreateMatrix4LPCQ(matrix,g,ge,p,pe,setpro,setmat,checkpro,seq,template,target,PSICtemplate,PSICtarget,qtemplate,qtarget,weight,data,cte,lambda1,tolerance)
...
float    weight[MAXM],data[2][MAXM][MAXE][MAXE],cte[MAXP];
{
   ...
   if (cte[n-1]!=0){
    ...
    for (i=0;i<lena;i++){
     for (ii=2;ii<MAXE;ii++){
       if (qtemplate[n][ii]>0.0 && PSICtemplate[n][i].targetQ[ii]>0.0){
       w1[i][ii]=log(PSICtemplate[n][i].targetQ[ii]/qtemplate[n][ii]);
       mean_w1[ii]+=w1[i][ii];
       }
     }
    }
    ...
    for (i=0;i<lenb;i++){
     for (ii=2;ii<MAXE;ii++){
       if (qtarget[n][ii]>0.0 && PSICtarget[n][i].targetQ[ii]>0.0){
       w2[i][ii]=log(PSICtarget[n][i].targetQ[ii]/qtarget[n][ii]);
       mean_w2[ii]+=w2[i][ii];
       }
     }
    }
    ...
    printf("lambda log(Pearsson) (Q)[%d] = %f\n",n,lambda2[n]);
   ...
   if (weight[k]!=0){
    ...
    for (i=0;i<lena;i++){
     for (ii=2;ii<MAXE;ii++){
       if (qtemplate[0][ii]>0.0 && PSICtemplate[n][i].targetQ[ii]>0.0){
       w1[i][ii]=log(PSICtemplate[n][i].targetQ[ii]/qtemplate[0][ii]);
       mean_w1[ii]+=w1[i][ii];
       }
     }
    }
    ...
    for (i=0;i<lenb;i++){
     for (ii=2;ii<MAXE;ii++){
       if (qtarget[0][ii]>0.0 && PSICtarget[n][i].targetQ[ii]>0.0){
       w2[i][ii]=log(PSICtarget[n][i].targetQ[ii]/qtarget[0][ii]);
       mean_w2[ii]+=w2[i][ii];
       }
     }
    }
