...
float  CreateMatrix4PICASSOQ(matrix,g,ge,p,pe,setpro,setmat,checkpro,seq,template,target,PSICtemplate,PSICtarget,qtemplate,qtarget,weight,data,cte,lambda1,tolerance)
...
float    weight[MAXM],data[2][MAXM][MAXE][MAXE],cte[MAXP];
{
      ...
      for (j=0;j<lenb;j++){
        ...
        for (ii=2;ii<MAXE;ii++){
          if (qtemplate[n][ii]>0.0 && qtarget[n][ii] > 0.0){
             if (PSICtemplate[n][i].targetQ[ii]>0.0) s+=PSICtarget[n][j].targetQ[ii]*log(PSICtemplate[n][i].targetQ[ii]/qtemplate[n][ii]);
             if (PSICtarget[n][j].targetQ[ii]>0.0 )  s+=PSICtemplate[n][i].targetQ[ii]*log(PSICtarget[n][j].targetQ[ii]/qtarget[n][ii]);
          }
        }
        score2[i][j] =  s ;
      }
     ...
     for (j=0;j<lenb;j++){
        ...
        for (ii=2;ii<MAXE;ii++){
          if (qtemplate[0][ii]>0.0 && qtarget[0][ii] > 0.0){
             if (PSICtemplate[n][i].targetQ[ii]>0.0) s+=PSICtarget[n][j].targetQ[ii]*log(PSICtemplate[n][i].targetQ[ii]/qtemplate[0][ii]);
             if (PSICtarget[n][j].targetQ[ii]>0.0 )  s+=PSICtemplate[n][i].targetQ[ii]*log(PSICtarget[n][j].targetQ[ii]/qtarget[0][ii]);
          }
        }
        score2[i][j] =  s ;
      }
   ...
   if (ctp>1.0) ctp=1.0;
/*
  for (ii=2;ii<MAXE;ii++){  printf(" PSICtarget[3][10].targetQ[%d] %f PSICtemplate[3][10].targetQ[%d] %f qtemplate[3][%d] %f qtarget[3][%d] %f \n",ii,PSICtarget[3][10].targetQ[ii],ii, PSICtemplate[3][10].targetQ[ii],ii,qtemplate[0][ii],ii,qtarget[0][ii]); }
  for (k=0;k<MAXM;k++){printf("weight[%d]=%f\n",k,weight[k]);}
*/
