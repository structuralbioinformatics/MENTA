...
float  CreateMatrix4raw(matrix,g,ge,p,pe,setpro,setmat,checkpro,seq,template,target,weight,data,cte,lambda1,tolerance)
     ...
     for (n=1;n<setpro;n++){
      for (ii=0;ii<template[n].size;ii++){
	   ...
	   if ( (jtgt > 1 && itmp >1) || (jtgt <=1 && itmp <= 1) ){
            matrix[j][i] +=  cte[n-1]* (1.0/template[n].size) * (1.0/target[n].size) * data[1][n-1][jtgt][itmp]*lambda1[n];
	   }else{
            matrix[j][i] -=  cte[n-1]* (1.0/template[n].size) * (1.0/target[n].size) * data[1][n-1][jtgt][itmp]*lambda1[n];
           }
       }
    ...
    for (k=0;k<setmat;k++){
     ...
     for (ii=0;ii<template[0].size;ii++){
      for (jj=0;jj<target[0].size;jj++){
	   ...
	   if ( (jtgt > 1 && itmp >1) || (jtgt <=1 && itmp <= 1) ){
            matrix[j][i] += ( 1.0 - ctp ) * weight[k] * (1.0/template[0].size) * (1.0/target[0].size) * data[0][k][jtgt][itmp]*lambda1[n];
	   }else{
            matrix[j][i] -= ( 1.0 - ctp ) * weight[k] * (1.0/template[0].size) * (1.0/target[0].size) * data[0][k][jtgt][itmp]*lambda1[n];
           }
      }
