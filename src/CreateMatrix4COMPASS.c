...
float  CreateMatrix4COMPASS(matrix,g,ge,p,pe,setpro,setmat,checkpro,seq,template,target,PSICtemplate,PSICtarget,qtemplate,qtarget,weight,data,cte,lambda1,tolerance)
      ...
      for (j=0;j<lenb;j++){
        ...
        for (ii=2;ii<MAXE;ii++){
          if (qtemplate[n][ii]>0.0 && qtarget[n][ii] > 0.0){
             if (PSICtemplate[n][i].targetQ[ii]>0.0) s+=PSICtarget[n][j].element[ii]*log(PSICtemplate[n][i].targetQ[ii]/qtemplate[n][ii]);
             if (PSICtarget[n][j].targetQ[ii]>0.0)   s+=PSICtemplate[n][i].element[ii]*log(PSICtarget[n][j].targetQ[ii]/qtarget[n][ii]);
          }
        }
        score2[i][j] =  s ;
      }
     ...
     for (j=0;j<lenb;j++){
        ...
        for (ii=2;ii<MAXE;ii++){
          if (qtemplate[0][ii]>0.0 && qtarget[0][ii] > 0.0){
             if (PSICtemplate[n][i].targetQ[ii]>0.0) s+=PSICtarget[0][j].element[ii]*log(PSICtemplate[n][i].targetQ[ii]/qtemplate[0][ii]);
             if (PSICtarget[n][j].targetQ[ii]>0.0)   s+=PSICtemplate[0][i].element[ii]*log(PSICtarget[n][j].targetQ[ii]/qtarget[0][ii]);
          }
        }
        score2[i][j] =  s ;
      }
